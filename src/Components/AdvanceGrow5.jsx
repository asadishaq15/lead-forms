...
const AutoInsuranceLeadForm = () => {
  const [formData, setFormData] = useState({
    ...
    jornaya: "",
    ip_address: "54.92.195.243", // Using the IP address from the attached document
    first_name: "",
    last_name: "",
    address: "",
    city: "",
  ...
  // Initialize form with IP address and Jornaya ID
  useEffect(() => {
  ...
  const handleSubmit = (e) => {
    ...
    // Prepare URL with parameters - this follows the approach from SSDICallTransfer
    ...
    // Get phone in E.164 format for callerid parameter
    ...
    fetch(url, {
      ...
      .then((data) => {
        ...
        if (data && data.status === "ok") {
          ...
          // Generate new Jornaya ID for next submission
  ...
  const handleChange = (e) => {
    ...
    // For date fields, ensure only digits and hyphens can be entered
    if ((name === 'dob' || name === 'policy_start_date') && value !== '') {
      // Allow only digits and hyphens
      const sanitized = value.replace(/[^\d-]/g, '');
      
      // Format automatically as YYYY-MM-DD
      let formatted = sanitized;
      if (sanitized.length === 4 && !sanitized.includes('-')) {
        formatted = `${sanitized}-`;
      } else if (sanitized.length === 7 && sanitized.split('-').length === 2) {
        formatted = `${sanitized}-`;
      }
